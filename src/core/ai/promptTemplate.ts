...
export async function initializePromptSystem(): Promise<void> {
  try {
    ...
    if (!fs.existsSync(PROMPTS_DIR)) {
      ...
      info(`Created prompts directory: ${PROMPTS_DIR}`);
    }

    // Load all prompts into cache
    await loadAllPrompts();
    info(`Initialized prompt system with ${promptCache.size} templates`);
  } catch (err) {
    error('Failed to initialize prompt system:', isError(err) ? err : String(err));
    throw err;
  }
}
  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        error(`Error loading prompt file ${file}:`, isError(err) ? err : String(err));
      }
    }

    return templates;
  } catch (err) {
    error('Error loading prompts:', isError(err) ? err : String(err));
    return [];
  }
}
...
export async function getPromptTemplate(name: string): Promise<PromptTemplate | null> {
  ...
  try {
    ...
    error(`Error loading prompt template ${name}:`, isError(err) ? err : String(err));
  }
...
export async function savePromptTemplate(template: PromptTemplate): Promise<boolean> {
  try {
    ...
    error(`Error saving prompt template ${template.name}:`, isError(err) ? err : String(err));
    return false;
  }
}
    ...
    for (const file of files) {
      ...
      if (!lastModified || stats.mtimeMs > lastModified) {
        try {
          ...
          info(`Reloaded prompt template: ${file}`);
        } catch (err) {
          error(`Error reloading prompt template ${file}:`, isError(err) ? err : String(err));
        }
      }
    }
  } catch (err) {
    error('Error checking for prompt updates:', isError(err) ? err : String(err));
  }
}
