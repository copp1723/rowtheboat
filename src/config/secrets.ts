...
export function initializeSecrets(key: string): boolean {
  try {
    ...
    if (isDefaultValue('ENCRYPTION_KEY', key)) {
      logger.warn('Using a default encryption key. This is insecure for production use.');
      
      if (process.env.NODE_ENV === 'production') {
        logger.error('Default encryption key detected in production environment. This is a security risk.');
        return false;
      }
    }
    ...
    logger.error('Failed to initialize encryption key for secrets', error);
    return false;
  }
}
